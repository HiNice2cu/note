...
# 主干网络（backbone）
backbone:
  [[-1, 1, Conv, [64, 6, 2, 2]],  # from=-1（上一层）, number=1（重复1次）, module=Conv, 参数=[输出通道64, 卷积核6, 步长2, padding=2]
   [-1, 1, Conv, [128, 3, 2]],
   [-1, 3, C3, [128]],  # C3模块重复3次
   [-1, 1, Conv, [256, 3, 2]],
   [-1, 6, C3, [256]],
...
head:
  [[-1, 1, Conv, [512, 1, 1]],
   ...
   [[-1, 6], 1, Concat, [1]],  # 拼接上一层与第6层输出
   [-1, 3, C3, [512, False]],
   [-1, 1, Conv, [256, 1, 1]],
   [-1, 1, nn.Upsample, [None, 2, 'nearest']],
   [[-1, 4], 1, Concat, [1]],  # 拼接上一层与第4层输出
   [-1, 3, C3, [256, False]],
   [-1, 1, Conv, [256, 3, 2]],
   [[-1, 14], 1, Concat, [1]],  # 拼接上一层与第14层输出
   [-1, 3, C3, [512, False]],
   [-1, 1, Conv, [512, 3, 2]],
   [[-1, 10], 1, Concat, [1]],  # 拼接上一层与第10层输出
   [-1, 3, C3, [1024, False]],
   [[17, 20, 23], 1, Detect, [nc, anchors]],  # 检测层，融合3个特征图
  ]
```
...
- **backbone/head**：网络核心部分，每一行配置格式为 `[from, number, module, args]`，含义如下：
    ...
    - args：模块参数（如 Conv 模块参数为 [输出通道、卷积核大小、步长、padding]）。

### 二、用 TensorBoard 可视化模型结构（实操方法）

网络结构可通过 TensorBoard 直观展示，无需手动绘图，同时支持监控训练参数，操作步骤如下（代码从原文提取，无修改）：
