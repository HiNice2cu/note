...
description = "Upload an image to detect objects using a custom-trained YOLOv5 model."
...
gr.Interface(inputs=[gr.Image(sources=["webcam", "upload"], label="选择输入源", type="numpy"),gr.Slider(minimum=0.0,maximum=1.0,value=base_conf),gr.Slider(minimum=0.0,maximum=1.0,value=base_iou)],
             outputs=["image"],
             #1.Gradio的设计哲学是"声明式"编程：你声明输入组件、函数和输出组件，Gradio负责连接它们
			 #2.fn= 参数指定的函数是Gradio自动调用的回调函数
			 #3.参数传递顺序完全由inputs列表的顺序决定
             fn=det_image,
             title=title,
